...
pub async fn wm(ctx: Context<'_>) -> Result<(), Error> {
...
pub async fn wmadd(
    ctx: Context<'_>,
    #[description = "Number of white monsters to add (default: 1)"] amount: Option<u32>,
) -> Result<(), Error> {
    let wm_counter = ctx
        .data()
    ...
    ctx.say(format!(
        "Another white monster for the boysss! {} / 32",
        wm_counter
    ))
    .await?;
    Ok(())
...
pub async fn wmrm(
    ctx: Context<'_>,
    #[description = "Number of white monsters to remove (default: 1)"] amount: Option<u32>,
) -> Result<(), Error> {
    let wm_counter = ctx
        .data()
    ...
    ctx.say(format!(
        "Removed one white monster from the counter! {} / 32",
        wm_counter
    ))
    .await?;
    Ok(())
